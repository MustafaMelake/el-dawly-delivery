...
import { UserPlus, Truck, Loader2 } from "lucide-react";
...
const SignUp = () => {
  ...
  return (
    <>
      <div
        className="flex justify-center items-center min-h-screen px-4 py-12 bg-gray-50 dark:bg-black"
        dir="rtl"
      >
        <div className="w-full max-w-md bg-white dark:bg-zinc-950 rounded-2xl shadow-xl p-8 border border-gray-200 dark:border-zinc-800">
          ...
          <p className="text-center text-gray-500 dark:text-zinc-400 text-sm mb-8">
            انضم لمكتب الدولي وابدأ في تتبع شحناتك بسهولة
          </p>

          {error && (
            <div className="bg-red-50 dark:bg-red-900/10 text-red-600 dark:text-red-400 p-3 rounded-xl text-sm text-center mb-6 border border-red-100 dark:border-red-900/20 font-medium">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <input
                required
                type="text"
                placeholder="مصطفى ملاك"
                className="w-full p-3 rounded-xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all dark:text-zinc-200"
            ...
            <button
              disabled={loading}
              type="submit"
              className="w-full py-4 mt-2 bg-primary hover:bg-primary/90 text-white font-bold rounded-xl transition-all shadow-lg shadow-primary/20 disabled:opacity-50"
            >
              {loading ? (
